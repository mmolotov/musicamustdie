...
test('переключает семейства, маршруты и анимирует выбранную аппликатуру', async ({ page }) => {
  ...
  const family = page.getByLabel('Семейство аппликатур')
  await expect(family).toHaveValue('recommended')
  // Подробности свёрнуты: раскрываем один раз, дальше панель остаётся открытой.
  await page.getByText('Подробнее об аппликатуре', { exact: true }).click()
  ...
  await family.selectOption('two-octave')
  ...
  await expect(page.locator('.pattern-line strong')).toHaveText('2–3–3–3–4 · 5-я стр.')
  ...
  await family.selectOption('3nps')
  await expect(page.locator('.pattern-line strong')).toHaveText('3NPS 1 · C')
  ...
  await family.selectOption('extended')
...
test('сохраняет профиль и оставляет CAGED на верхних шести струнах восьмиструнной гитары', async ({ page }) => {
  ...
  await page.getByLabel('Семейство аппликатур').selectOption('caged')
  await expect(page.locator('.pattern-chip')).toHaveCount(5)
...
test('мобильная версия: гриф гаммы прокручивается вбок, а таб следует за воспроизведением', async ({ page }) => {
  ...
  await page.getByLabel('Семейство аппликатур').selectOption('3nps')
  await page.getByRole('radio', { name: '2 октавы' }).click()
  await page.getByText('Подробнее об аппликатуре', { exact: true }).click()
  await expect(page.getByText(/Маршрут: C → C → C/)).toBeVisible()
