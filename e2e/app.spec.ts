...
test('выбирает формы из библиотеки, меняет маршруты и анимирует проигрыш', async ({ page }) => {
  await page.goto('/')
  ...
  // На виду только формы, покрывающие гриф; остальные — под раскрытием.
  const recommended = page.locator('.pattern-picker').first().locator('.pattern-chip')
  await expect(recommended).toHaveCount(7)
  await expect(recommended.filter({ hasText: /Форма E|Позиционная 4/ })).toHaveCount(1)
  await expect(page.locator('.pattern-line')).toContainText('Форма E')
  await expect(page.locator('.pattern-line')).toContainText('Маршрут: C → C → C')

  await page.getByText('Все аппликатуры', { exact: true }).click()
  const library = page.locator('.library-sections')
  await expect(library.locator('section')).toHaveCount(6)
  await expect(library.locator('.pattern-chip')).toHaveCount(33)

  const caged = library.locator('section').filter({ hasText: 'CAGED' }).first()
  await expect(caged.locator('.pattern-chip')).toHaveCount(5)
  await caged.locator('.pattern-chip').filter({ hasText: 'Форма C' }).click()
  await expect(page.locator('.pattern-line strong').first()).toHaveText('Форма C')
  await expect(caged.locator('.pattern-chip').filter({ hasText: 'Форма C' })).toContainText('0–3 лады')
  await page.getByRole('radio', { name: '1 октава' }).click()
  await expect(page.locator('.pattern-line')).toContainText('Маршрут: C → C')
  await expect(page.locator('[data-tab-step]')).toHaveCount(8)
  await expect(page.locator('.fret-note[data-finger]').first()).toBeVisible()

  const twoOctave = library.locator('section').filter({ hasText: '2 октавы' }).first()
  await expect(twoOctave.locator('.pattern-chip')).toHaveCount(3)
  const flexibleTwoOctave = twoOctave.locator('.pattern-chip').filter({ hasText: '2–3–3–3–4' })
  ...
  await expect(page.locator('.pattern-line strong').first()).toHaveText('2–3–3–3–4 · 5-я стр.')
  await expect(page.locator('.pattern-line')).toContainText('Маршрут: C → C → C')
  await expect(page.locator('[data-tab-step]')).toHaveCount(15)

  await twoOctave.locator('.pattern-chip').filter({ hasText: '1–3–3–3–3–2' }).click()
  await expect(page.locator('.pattern-line strong').first()).toHaveText('1–3–3–3–3–2 · 6-я стр.')
  await expect(page.locator('[data-tab-step]')).toHaveCount(15)

  const threeNps = library.locator('section').filter({ hasText: '3NPS' }).first()
  await threeNps.locator('.pattern-chip').filter({ hasText: '3NPS 1' }).click()
  await expect(page.locator('.pattern-line strong').first()).toHaveText('3NPS 1 · C')
  ...
  const extended = library.locator('section').filter({ hasText: 'Расширенные' }).first()
  await extended.locator('.pattern-chip').filter({ hasText: '3 октавы' }).click()
  await page.getByRole('radio', { name: '3 октавы' }).click()
  await expect(page.locator('.pattern-line')).toContainText('Маршрут: C → C → C → C')
...
test('сохраняет профиль и оставляет CAGED на верхних шести струнах восьмиструнной гитары', async ({ page }) => {
  ...
  await page.getByText('Все аппликатуры', { exact: true }).click()
  const caged = page.locator('.library-sections section').filter({ hasText: 'CAGED' }).first()
  await expect(caged.locator('.pattern-chip')).toHaveCount(5)
  await caged.locator('.pattern-chip').first().click()
...
test('мобильная версия: гриф гаммы прокручивается вбок, а таб следует за воспроизведением', async ({ page }) => {
  ...
  await page.getByText('Все аппликатуры', { exact: true }).click()
  await page.locator('.library-sections section').filter({ hasText: '3NPS' }).first()
    .locator('.pattern-chip').filter({ hasText: '3NPS 1' }).click()
  await page.getByRole('radio', { name: '2 октавы' }).click()
  await expect(page.locator('.pattern-line')).toContainText('Маршрут: C → C → C')
